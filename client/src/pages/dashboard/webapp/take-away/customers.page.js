...
import ListCustomersWebapp from "@/components/dashboard/webapp/_shared/list-customers.webapp";

export default function TakeAwayWebappCustomersPage() {
  ...
  const employeeHasAccess =
    ...
    isEmployeeDashboardRouteAllowed("/dashboard/webapp/take-away/customers", {
        ...
        <div className="gm-webapp-scroll-container flex h-[100dvh] flex-1 flex-col gap-6 overflow-y-auto overscroll-none bg-lightGrey p-6 px-2 text-darkBlue mobile:px-6">
          {!hasTakeAwayModule ? (
            ...
            <ListCustomersWebapp
              ...
              lockSourceFilter
              sidebarTitle="Vente à emporter"
              sidebarModule="take_away"
            />
          )}
        </div>
      </div>
...
export async function getStaticProps({ locale }) {
  return {
    props: {
      ...(await serverSideTranslations(locale, ["common", "customers"])),
    },
  };
}
