...
import AddWinesComponent from "@/components/dashboard/wines/add.wines.component";
        ...
        <div className="flex">
          <NavComponent />
           ...
           <div className="tablet:ml-[270px] bg-lightGrey text-darkBlue flex-1 p-6 flex flex-col gap-6 min-h-screen">
            ...
            <AddWinesComponent category={props.category} wine={props.wine} />
          </div>
        </div>
      </div>
    </>
...
export async function getServerSideProps({ params, query, locale }) {
  ...
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/categories/${categoryId}/wines`
    );
    const { category } = response.data;

    let wine = null;
    if (wineId) {
      wine = category.wines.find((d) => d._id === wineId) || null;
    }
