...
import CategoriesListWinesComponent from "@/components/dashboard/wines/categories-list.wines.component";
import NoAvailableComponent from "@/components/_shared/options/no-available.options.component";

export default function WinesPage(props) {
  ...
  const hasWinesModule = !!restaurantOptions.wines;

  const user = restaurantContext.userConnected;
  const isEmployee = user?.role === "employee";

  let employeeHasWinesAccess = true;
  ...
  if (isEmployee && restaurant) {
    ...
    employeeHasWinesAccess = profile?.options?.wines === true;
  }
        ...
        <div className="flex">
          ...
          <div className="tablet:ml-[270px] bg-lightGrey text-darkBlue flex-1 px-2 p-6 mobile:p-6 mobile:px-6 flex flex-col gap-6 min-h-screen">
            ...
            {!hasWinesModule ? (
              ...
              />
            ) : !employeeHasWinesAccess ? (
              <NoAvailableComponent
                dataLoading={restaurantContext.dataLoading}
                emptyText="Vous n'avez pas accès à cette section"
              />
            ) : (
              <CategoriesListWinesComponent />
            )}
          </div>
        </div>
      </div>
