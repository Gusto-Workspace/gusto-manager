...
import TrainingSessionsForm from "@/components/dashboard/health-control-plan/training-sessions/form.component";
import TrainingSessionsList from "@/components/dashboard/health-control-plan/training-sessions/list.component";
...
export default function TrainingSessionsPage(props) {
            ...
            {restaurantContext?.restaurantData?.options?.health_control_plan ? (
              <section className="flex flex-col gap-6">
                ...
                <CatalogHeaderDashboardComponent
                  ...
                  subtitle="Formation du personnel"
                />

                <div className="flex flex-col gap-6">
                  <TrainingSessionsForm
                    restaurantId={restaurantContext.restaurantData?._id}
                    initial={editing}
                    onSuccess={(doc) => {
                      setEditing(null);
                      window.dispatchEvent(
                        new CustomEvent("training-sessions:upsert", {
                          detail: { doc },
                        }),
                      );
                    }}
