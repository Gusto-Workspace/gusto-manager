...
const DEFAULT_TEMPLATE = "Bonjour {firstName}, rappel de votre reservation chez {restaurantName} le {date} a {time} pour {guests} pers.";
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.NEXT_PUBLIC_API_URL}/restaurants/${restaurantId}/sms-reminders`)
      .then(({ data }) => {
        ...
        setSettings((current) => ({ ...current, ...(data.settings || {}), sender: { ...current.sender, ...(data.settings?.sender || {}) } }));
  ...
  return (
    <section className="rounded-2xl border border-darkBlue/10 bg-white p-4 shadow-sm md:p-6">
      <div className="mb-5 flex items-start gap-3">
        ...
        <div><h2 className="font-semibold text-darkBlue">Rappels SMS</h2><p className="text-sm text-darkBlue/60">100 crédits inclus par période Stripe, puis 0,10 € par crédit.</p></div>
      </div>
      {loading ? <Loader2 className="size-5 animate-spin text-darkBlue/60" /> : !subscribed ? (
        ...
        <div className="grid gap-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="grid gap-1 text-sm">Délai avant la réservation<select value={settings.delayMinutes} onChange={(event) => update("delayMinutes", Number(event.target.value))} className="rounded-xl border border-darkBlue/15 px-3 py-2"><option value={120}>2 heures</option><option value={360}>6 heures</option><option value={720}>12 heures</option><option value={1440}>1 jour</option><option value={2880}>2 jours</option></select></label>
          ...
          <label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={settings.internationalEnabled} onChange={(event) => update("internationalEnabled", event.target.checked)} className="size-4" />Autoriser les destinations internationales configurées par Gusto (elles peuvent consommer plusieurs crédits).</label>
