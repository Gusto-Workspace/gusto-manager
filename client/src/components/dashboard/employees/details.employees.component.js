...
export default function DetailsEmployeesComponent({ employeeId }) {
  ...
  const onSaveOptions = async (data) => {
    try {
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      {/* Details & Photo */}
      <section className="bg-white p-6 rounded-lg shadow flex justify-between items-center relative">
        {!isEditing && (
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        >
          ...
          <div className="relative w-32 h-32 rounded-full overflow-hidden border border-darkBlue/20 flex-shrink-0">
            {previewUrl ? (
              <img
                src={previewUrl}
                alt="aperçu"
