...
export default function CardCategoryListComponent(props) {
  ...
  const totalWinesCount =
    (props.category.wines ? props.category.wines.length : 0) +
    (props.category.subCategories
      ? props.category.subCategories.reduce(
          (count, subCategory) =>
            count + (subCategory.wines ? subCategory.wines.length : 0),
          0
        )
      : 0);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-4 justify-between h-full">
        <h2 className="text-base mx-auto px-8 text-center tablet:text-lg font-semibold text-darkBlue text-balance">
          {props.category.name}
        </h2>

        <div className="flex flex-col items-center">
          <p className="text-xs text-darkBlue/60">
            {t("labels.numberOfWines")} :{" "}
            <span className="font-medium text-darkBlue">{totalWinesCount}</span>
          </p>
