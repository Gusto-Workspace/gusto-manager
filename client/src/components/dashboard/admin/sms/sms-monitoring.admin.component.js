  ...
  return (
    <section className="grid gap-6">
      ...
      {loading ? <Loader2 className="size-5 animate-spin" /> : error ? <p className="text-red-600">{error}</p> : (
        <>
          <div className="grid gap-3 midTablet:grid-cols-2 desktop:grid-cols-3"><div className="rounded-2xl bg-white p-4"><span className="text-sm text-darkBlue/55">Jobs visibles</span><strong className="block text-2xl">{data.jobs.length}</strong></div><div className="rounded-2xl bg-white p-4"><span className="text-sm text-darkBlue/55">Uncertain</span><strong className="block text-2xl">{data.jobs.filter((job) => job.status === "uncertain").length}</strong></div><div className="rounded-2xl bg-white p-4"><span className="text-sm text-darkBlue/55">Sender IDs à traiter</span><strong className="block text-2xl">{pendingSenders.length}</strong></div></div>
