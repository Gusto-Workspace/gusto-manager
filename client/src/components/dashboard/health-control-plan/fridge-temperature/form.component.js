...
function buildFormDefaults(record) {
  return {
    ...
    // signature non gérée dans ce 1er jet de formulaire (facile à ajouter)
  };
}
...
export default function FridgeTemperatureForm({
  ...
  const [knownFridges, setKnownFridges] = useState([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_URL}/restaurants/${restaurantId}/fridge-temperatures/distinct/fridges`;
        const { data } = await axios.get(url, {
          headers: { Authorization: `Bearer ${token}` },
          params: { limit: 200 },
        });
        if (!cancelled) {
          setKnownFridges(Array.isArray(data?.items) ? data.items : []);
        }
      } catch (e) {
        if (!cancelled) setKnownFridges([]);
        console.error(e);
      } finally {
  ...
  const fridgeSuggestions = useMemo(() => {
    const uniq = new Map();
    for (const it of knownFridges || []) {
      const key = `${it.fridgeName || ""}||${it.location || ""}`;
