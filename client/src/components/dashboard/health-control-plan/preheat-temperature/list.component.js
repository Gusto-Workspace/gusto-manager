// components/dashboard/health-control-plan/preheat-temperature-list.component.jsx
"use client";
...
/* --------------------------- Labels FR --------------------------- */
const PHASE_LABELS = {
  preheat: "Mise en chauffe",
  "hot-holding": "Maintien au chaud",
};

/* --------------------------- Component --------------------------- */
export default function PreheatTemperatureList({
  restaurantId,
  onEdit,
  editingId = null,
  onDeleted,
  ...
  const fetchData = async (page = 1, overrides = {}) => {
    ...
    try {
      ...
      console.error("fetch preheat list error:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener("preheat-temperature:upsert", handleUpsert);
    return () =>
  ...
  const onConfirmDelete = async () => {
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL}/restaurants/${restaurantId}/preheat-temperatures/${deleteTarget._id}`;
      ...
      <div className="mb-4 grid gap-2 grid-cols-1 midTablet:grid-cols-[1fr_auto_auto]">
        ...
        <div className={fieldWrap}>
          ...
          <input
            ...
            placeholder="Équipement, emplacement, phase, unité, opérateur, note…"
            className={inputCls}
          />
        </div>
