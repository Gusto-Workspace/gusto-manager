...
export default function MicrobiologyList({
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("microbiology:upsert", handleUpsert);
  }, [restaurantId]);
                ...
                >
                  ...
                  <td className="py-2 pr-3 whitespace-nowrap">
                    {typeof it.passed === "boolean" ? (
                      <span className={`px-2 py-0.5 rounded text-xs ${it.passed ? "bg-green text-white" : "bg-red text-white"}`}>
                        {it.passed ? "Conforme" : "Non conforme"}
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded text-xs bg-orange text-white">Indéfini</span>
                    )}
                  </td>
                  ...
                  <td className="py-2 pr-3 whitespace-nowrap">
                    {it.reportUrl ? <a href={it.reportUrl} target="_blank" rel="noreferrer" className="text-blue underline">Ouvrir</a> : "—"}
                  </td>
                  <td className="py-2 pr-3 whitespace-nowrap">
                    {it?.recordedBy
