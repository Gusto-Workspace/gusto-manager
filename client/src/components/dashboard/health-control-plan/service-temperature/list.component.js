...
/** On normalise en kebab-case pour matcher les clés avec tirets (ex: room-service) */
...
function translateServiceType(v) {
  const k = normalizeKebab(v);
  return SERVICE_TYPE_LABELS[k] ?? (v || "");
}

function translateServingMode(v) {
  const k = normalizeKebab(v);
  return SERVING_MODE_LABELS[k] ?? (v || "");
}

/** Concatène proprement : "Chaud / À emporter" ; si un seul dispo, on n'affiche que celui-ci ; si aucun → "—" */
function formatModeCell(serviceType, servingMode) {
  const left = translateServiceType(serviceType);
  const right = translateServingMode(servingMode);
...
export default function ServiceTemperatureList({
  ...
  const fetchData = async (page = 1, overrides = {}) => {
    ...
    try {
      ...
      console.error("fetch service list error:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="rounded-2xl border border-darkBlue/10 bg-white p-4 midTablet:p-5 shadow">
      ...
      <div className="flex flex-col midTablet:flex-row gap-2 mb-2">
        ...
        <div className={`${fieldWrap} w-full`}>
          ...
          <input
            ...
            placeholder="Zone, plat, identifiants, mode/type, opérateur, note…"
            className={inputCls}
          />
        </div>
      ...
      <div className="relative">
        <div className="overflow-x-auto max-w-[calc(100vw-83px)] midTablet:max-w-[calc(100vw-92px)] tablet:max-w-[calc(100vw-370px)] rounded-xl border border-darkBlue/10">
