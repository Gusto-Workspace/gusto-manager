...
/* -------------------- DISTINCT FRIDGES (pour select facultatif) -------------------- */
router.get(
  "/restaurants/:restaurantId/fridge-temperatures/distinct/fridges",
  authenticateToken,
  async (req, res) => {
    try {
      const { restaurantId } = req.params;
      const rows = await FridgeTemperature.aggregate([
        { $match: { restaurantId } },
        {
          $group: {
            _id: {
        ...
        { $sort: { "_id.fridgeName": 1 } },
      ]);
      const items = rows.map((r) => ({
        fridgeName: r._id.fridgeName || "",
        fridgeId: r._id.fridgeId || "",
      ...
      console.error("GET /fridge-temperatures/distinct/fridges:", err);
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des enceintes" });
    }
  }
);
