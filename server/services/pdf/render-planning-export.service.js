...
async function buildPlanningExportPdf({
  ...
  return new Promise((resolve, reject) => {
    ...
    const globalShiftCount = employees.reduce(
      (total, item) => total + Number(item?.totals?.shiftCount || 0),
      0,
    );
    const globalMinutes = employees.reduce(
      (total, item) => total + Number(item?.totals?.plannedMinutes || 0),
      0,
    );
    ...
    doc
      ...
      .fontSize(21)
      .fillColor(COLORS.darkBlue)
      .text("Export du planning salariés");
    ...
    doc
      ...
      .text(`Créneaux: ${globalShiftCount}`, summaryBoxX + 14, summaryBoxY + 46)
      .text(
        `Temps planifié: ${formatMinutes(globalMinutes)}`,
        summaryBoxX + 14,
        summaryBoxY + 62,
      );
    ...
    employees.forEach((employeeReport, index) => {
      ...
      const totals = employeeReport?.totals || {};
      const dayRows = employeeReport?.days || [];
      ...
      doc
        ...
        .fillColor(COLORS.text)
        .text(
          `Créneaux: ${totals.shiftCount || 0}`,
          doc.page.margins.left,
          doc.y,
        )
        .text(
          `Temps planifié: ${formatMinutes(totals.plannedMinutes)}`,
          doc.page.margins.left,
        );
      ...
      if (!dayRows.length) {
        doc
          ...
          .fontSize(10)
          .fillColor(COLORS.text)
          .text("Aucun créneau sur la période sélectionnée.");
        return;
      }
      ...
      drawTable(
        doc,
        ["Jour", "Créneaux", "Nb", "Temps planifié"],
        rows,
        [92, 262, 48, 96],
      );
    });
